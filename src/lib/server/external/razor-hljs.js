/*
 * Repo: https://github.com/highlightjs/highlightjs-cshtml-razor
 */
...
export default function (hljs) {
	...
	const csbraces = {
		// allows to find exactly last closing brace in code blocks (to process codeblock content with "csharp" sub language)
		begin: '{',
		end: '}',
		contains: ['self'],
		skip: true
	...
	const quotes = {
		// allows to skip razor symbols/tags inside CS strings
		variants: [
			{ begin: /"/, end: /"/, skip: true },
			{ begin: /'/, end: /'/, skip: true }
		],
	...
	const razorCodeBlockVariants = ['for', 'if', 'switch', 'while', 'using', 'lock', 'foreach'].map(
		(keyword) => ({
			begin: `@${keyword}(?![\\w\\d])[^{]*\\{`,
			end: '}'
		})
	...
	[razorBlock, razorCodeBlock, razorTryBlock].forEach(function (mode) {
